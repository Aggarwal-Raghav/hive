...
COMMON_JVM_OPTS="-Djava.io.tmpdir=/tmp \
  ...
  -XX:+UseG1GC"
  # -agentpath:$HOME/Desktop/software/async-profiler-4.4-macos/lib/libasyncProfiler.dylib=start,event=cpu,alloc=2m,file=$HIVE_LOG_DIR/${SERVICE}_profile.jfr
...
case "$SERVICE" in
  ...
  metastore)
    ...
    export HADOOP_CLIENT_OPTS="$HADOOP_CLIENT_OPTS $COMMON_JVM_OPTS -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5006 -javaagent:$HIVE_HOME/lib/jmx_prometheus_javaagent-1.0.1.jar=8082:$HIVE_CONF_DIR/jmx_config.yaml"
    ;;

  hiveserver2)
    export HADOOP_HEAPSIZE=2048
    export HADOOP_CLIENT_OPTS="$HADOOP_CLIENT_OPTS $COMMON_JVM_OPTS -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5007 -javaagent:$HIVE_HOME/lib/jmx_prometheus_javaagent-1.0.1.jar=8081:$HIVE_CONF_DIR/jmx_config.yaml"
    ;;
esac
